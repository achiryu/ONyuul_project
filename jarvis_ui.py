...
COLOR_PRIMARY = "#3498DB"           # 주요 액션 (재연결, 파란 버튼 등)

COLOR_SUCCESS = "#2ECC71"           # 성공/완료를 나타내는 텍스트·상태
...
COLOR_WARNING = "#F39C12"           # 주의/진행중 표시
COLOR_WARNING_ALT = "#F1C40F"       # 대기 상태 등 노란 계열 강조
...
# ─────────────────────────────────────────────────────────────────────────
APP_VERSION = "1.0.0"
UPDATE_CHECK_URL = "https://raw.githubusercontent.com/YOUR_USERNAME/YOUR_REPO/main/version.json"
...
class ONyuulGUI(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        self.mic_frame.pack(side="left", padx=20, pady=10)
        ...
        self.mic_level_bar = ctk.CTkProgressBar(self.mic_frame, width=130, height=10, progress_color=COLOR_SUCCESS)
        ...
        self.mic_mute_btn.pack(side="left", padx=(10, 0))
        ...
        self.tab_sidebar.pack(side="right", fill="y", padx=(10, 0))
        self.tab_sidebar.pack_propagate(False)

        # 고정폭 사이드바를 먼저 pack해서 자리를 확보한 뒤, 나머지 공간을
        # content_container가 채우도록 순서를 지킨다. (expand=True인 위젯을
        # 먼저 pack하면 사이드바가 들어갈 공간이 안 남을 수 있다 — 기존
        # left_panel/tabview 구성도 같은 순서였다.)
    ...
    def open_api_key_dialog(self, on_saved=None, first_run: bool = False):
        ...
        ctk.CTkButton(btn_frame, text="테스트", width=90, command=do_test).pack(side="left", padx=5)
        ctk.CTkButton(
